...
class Cryptopia extends Exchange
{
    ...
    private function apiCall($method, array $req = array())
    {
        ...
        $private_set = array("GetBalance", "SubmitTransfer", "GetTransactions", "SubmitWithdraw", "GetDepositAddress", "GetOpenOrders", "GetTradeHistory", "GetTransactions", "SubmitTrade", "CancelTrade", "SubmitTip");
        static $ch = null;
        $ch = curl_init();
    ...
    {
        $result = $this->apiCall("GetBalance", array('Currency' => $currency ?: "")); // "" for All currency balances
    ...
    Public function submitWithdraw($currency, $currencyId, $address, $paymentId = null, $amount)
    {
        $result = $this->apiCall("SubmitWithdraw", array('Currency' => $currency, 'CurrencyId' => $currencyId, 'Address' => $address, 'PaymentId' => $paymentId, 'Amount' => $amount)); // "" for All currency balances
        $result = json_decode($result, true);
        if ($result['Success'] == "true") {
            // @todo ADD CODE TO REFORMAT Array to standard
            return $result['Data'];
    ...
    Public function submitTransfer($currency, $currencyId, $username, $amount)
    {
        $result = $this->apiCall("SubmitTransfer", array('Currency' => $currency, 'CurrencyId' => $currencyId, 'Username' => $username, 'Amount' => $amount)); // "" for All currency balances
        $result = json_decode($result, true);
        if ($result['Success'] == "true") {
            // @todo ADD CODE TO REFORMAT Array to standard
            return $result['Data'];
    ...
    Public function getTransactions($type, $count = null)
    {
        $result = $this->apiCall("GetTransactions", array('Type' => $type, 'Count' => $count)); // "" for All currency balances
        $result = json_decode($result, true);
        if ($result['Success'] == "true") {
            // @todo ADD CODE TO REFORMAT Array to standard
            return $result['Data'];
